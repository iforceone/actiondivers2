...
const ServiceRequest: React.FC<{ mode: RequestMode }> = ({ mode }) => {
  ...
  return <main className="mx-auto max-w-6xl px-5 pb-28 pt-32 sm:px-8 lg:pt-40">
    ...
    <form onSubmit={submit} className="mt-12 grid gap-12 lg:grid-cols-[minmax(0,1fr)_330px]">
      ...
      <aside><div className="sticky top-28 rounded-2xl bg-[#06212a] p-6"><h2 className="text-xl font-extrabold">Request summary</h2><p className="mt-5 font-bold">{selected?.name ?? 'Select a service'}</p>{selected && (selected.minimumPaidParticipants || selected.maxParticipants) && <p className="mt-2 text-sm leading-relaxed text-[#F8F4E8]/65">{selected.minimumPaidParticipants ? `Minimum billed guests: ${selected.minimumPaidParticipants}.` : ''}{selected.minimumPaidParticipants && selected.maxParticipants ? ' ' : ''}{selected.maxParticipants ? `Maximum guests: ${selected.maxParticipants}.` : ''}</p>}<div className="mt-5 flex justify-between border-y border-white/10 py-4 text-sm"><span className="text-[#F8F4E8]/65">Starting estimate</span><strong>{formatUsd(estimate)}</strong></div>{mode === 'transfer' && <p className="mt-4 text-sm leading-relaxed text-[#F8F4E8]/58">Transfer pricing is proposed and subject to staff confirmation.</p>}{error && <p role="alert" className="mt-5 rounded-xl bg-red-400/10 p-4 text-sm text-red-100">{error}</p>}<button disabled={submitting || !selected} className="mt-6 inline-flex min-h-12 w-full items-center justify-center rounded-full bg-[var(--brand-orange)] px-5 font-bold text-white disabled:opacity-45">{submitting ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Sending…</> : `Send ${mode} request`}</button><a href={whatsapp} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex min-h-12 w-full items-center justify-center rounded-full border border-white/15 px-5 text-sm font-bold"><MessageCircle className="mr-2 h-5 w-5" /> WhatsApp fallback</a></div></aside>
    </form>
  </main>;
};
