...
const Admin: React.FC = () => {
  ...
  const sendQuote = (payable: boolean) => detail && run(payable ? 'payment' : 'update', async () => {
    ...
    const sendDetail = draftBody.detail;
    const body = await apiFetch<{ detail: ReservationDetail; emailStatus: string }>(`/admin-api/reservations/${detail.reservation.id}/${payable ? 'send-for-payment' : 'send-update'}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', 'If-Match': String(sendDetail.reservation.version), 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({ validForDays }),
    });
              ...
              {!detail ? <div className="flex min-h-[420px] items-center justify-center border-y border-white/10 text-center text-[#F8F4E8]/48">Select a reservation to review it.</div> : (
                <div className="space-y-9">
                  ...
                  <section className="grid gap-4 border-t border-white/10 pt-8 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <button onClick={() => sendQuote(false)} disabled={quoteLines.length === 0 || Boolean(working)} className="inline-flex min-h-12 items-center justify-center rounded-xl border border-[#11C7D9]/35 px-4 text-sm font-bold text-[#BDF5FA] hover:bg-[#11C7D9]/10 disabled:cursor-not-allowed disabled:opacity-35"><Send className="mr-2 h-4 w-4" /> Send update</button>
                    <div><label className="mb-2 flex items-center justify-between gap-3 text-xs font-semibold text-[#D9EEF1]">Quote valid days<input type="number" min={1} max={30} value={validForDays} onChange={(event) => setValidForDays(Math.min(30, Math.max(1, Number(event.target.value) || 1)))} className="w-16 rounded-lg border border-white/15 bg-[#061a22] px-2 py-1.5 text-center font-bold text-[#F8F4E8] [color-scheme:dark] outline-none focus:border-[#11C7D9]" /></label><button onClick={() => sendQuote(true)} disabled={!paymentsEnabled || quoteLines.length === 0 || Boolean(working)} title={paymentsEnabled ? 'Save the current quote and email its payment link' : 'Payments are disabled until Belize Bank setup is approved'} className="inline-flex min-h-12 w-full items-center justify-center rounded-xl bg-[var(--brand-orange)] px-4 text-sm font-bold text-white hover:bg-[var(--brand-orange-light)] disabled:cursor-not-allowed disabled:opacity-35"><DollarSign className="mr-2 h-4 w-4" /> {paymentsEnabled ? 'Send quote & payment' : 'Payment setup pending'}</button></div>
                    <p className="text-xs leading-relaxed text-[#B7D2D7] sm:col-span-2 xl:col-span-4">Sending an update automatically saves the current quote draft first. Payment links remain unavailable until the bank gateway is approved and enabled.</p>
                  </section>
...
const DailyRosterPanel: React.FC<{ catalog: BookingCatalog; preview: boolean }> = ({ catalog, preview }) => {
  ...
  const activeItems = [...catalog.items].filter((item) => item.active && item.category !== 'Transfer').sort((a, b) => a.sortOrder - b.sortOrder);
  const selectedItems = activeItems.filter((item) => tours.includes(item.id));
  const groupIds = (categories: BookingCatalogItem['category'][]) => activeItems.filter((item) => categories.includes(item.category)).map((item) => item.id);
  ...
  const printRoster = () => {
    ...
    popup.opener = null;
    popup.document.write(`<!doctype html><html><head><title>Action Divers roster — ${escape(date)}</title><style>body{font:13px Arial,sans-serif;color:#10272d;margin:32px}h1{margin:0 0 6px;font-size:24px}p{margin:4px 0 20px;color:#49666d}table{width:100%;border-collapse:collapse}th,td{padding:9px 8px;border-bottom:1px solid #ccd8da;text-align:left;vertical-align:top}th{font-size:11px;text-transform:uppercase;color:#49666d}.summary{margin:18px 0;font-weight:bold}@media print{body{margin:16px}}</style></head><body><h1>Action Divers Daily Roster</h1><p>${escape(rosterTitle)} · ${escape(date)}</p><div class="summary">${countLabel(totals.reservations, 'reservation')} · ${countLabel(totals.adults, 'adult')} · ${countLabel(totals.children, 'child', 'children')} · ${countLabel(totals.guests, 'tour place')}</div><table><thead><tr><th>Activity</th><th>Guest / reference</th><th>Participation</th><th>Contact</th><th>Accommodation</th><th>Status</th><th>Notes</th></tr></thead><tbody>${rows.map((row) => `<tr><td>${escape(row.tour_name)}</td><td><strong>${escape(row.customer_name)}</strong><br>${escape(row.reference)}</td><td>${countLabel(row.adults, 'adult')}<br>${countLabel(row.children, 'child', 'children')}</td><td>${escape(row.customer_email)}<br>${escape(row.customer_phone || '')}</td><td>${escape(row.accommodation || '')}</td><td>${escape(STATUS_LABELS[row.status] || row.status)}</td><td>${escape(row.customer_notes || '')}${row.internal_notes ? `<br><strong>Staff:</strong> ${escape(row.internal_notes)}` : ''}</td></tr>`).join('')}</tbody></table></body></html>`);
...
const CompactCatalogPanel: React.FC<{ catalog: BookingCatalog; dirty: boolean; working: boolean; updateItem: (id: string, changes: Partial<BookingCatalogItem>) => void; save: () => false | Promise<void>; publish: () => Promise<void> }> = ({ catalog, dirty, working, updateItem, save, publish }) => {
  ...
  return <section className="mt-7">
    ...
    {!editing && <div className="mt-5 divide-y divide-white/10">{items.map((item) => <div key={item.id} className="grid gap-2 py-4 sm:grid-cols-[minmax(0,1fr)_170px_150px_110px] sm:items-center"><div><p className="font-bold">{item.name}</p><p className="mt-1 text-xs text-[#F8F4E8]/58">{item.pricingBasis === 'per_person' ? 'Per person' : item.pricingBasis === 'tiered_transfer' ? 'Tiered transfer' : 'Per group'}{item.pricingBasis === 'per_person' && item.minimumPaidParticipants ? ` · Minimum billed ${item.minimumPaidParticipants}` : ''}{item.maxParticipants ? ` · Maximum ${item.maxParticipants} guests` : ''}</p></div><div className="text-sm text-[#B7D2D7]"><span className="block font-semibold text-[#D9EEF1]">{item.startTime || 'Start time not set'}</span><span className="mt-1 block text-xs">{item.duration || 'Duration not set'}</span></div><span className="font-bold">{formatUsd(item.priceCents)}</span><span className={`w-fit rounded-full px-2 py-1 text-xs font-bold ${item.active ? 'bg-emerald-300/10 text-emerald-200' : 'bg-white/8 text-[#B7D2D7]'}`}>{item.active ? 'Active' : 'Inactive'}</span></div>)}</div>}
    <AnimatedDisclosure open={editing} id="catalog-editor"><div className="pt-5"><p className="mb-2 text-sm leading-relaxed text-[#F8F4E8]/65">Edit operational schedule details alongside pricing and participant limits. Leave a schedule field blank when staff must confirm it after booking.</p><div className="divide-y divide-white/10">{items.map((item) => <div key={item.id} className="grid gap-4 py-5 md:grid-cols-2 xl:grid-cols-4 xl:items-end"><label className={`${labelClass} xl:col-span-2`}>Public name<input value={item.name} onChange={(event) => updateItem(item.id, { name: event.target.value })} className={fieldClass} /></label><label className={labelClass}>Start time<input value={item.startTime ?? ''} maxLength={100} placeholder="Confirmed after booking" onChange={(event) => updateItem(item.id, { startTime: event.target.value || undefined })} className={fieldClass} /></label><label className={labelClass}>Duration<input value={item.duration ?? ''} maxLength={120} placeholder="Duration not published" onChange={(event) => updateItem(item.id, { duration: event.target.value || undefined })} className={fieldClass} /></label><label className={labelClass}>Price USD<input inputMode="decimal" value={(item.priceCents / 100).toFixed(2)} onChange={(event) => updateItem(item.id, { priceCents: Math.max(0, Math.round(Number(event.target.value) * 100) || 0) })} className={fieldClass} /></label><label className={labelClass}>Pricing<select value={item.pricingBasis} onChange={(event) => updateItem(item.id, { pricingBasis: event.target.value as BookingCatalogItem['pricingBasis'], ...(event.target.value === 'per_person' ? {} : { minimumPaidParticipants: undefined }) })} className={fieldClass}><option value="per_person">Per person</option><option value="per_group">Per group</option>{item.pricingBasis === 'tiered_transfer' && <option value="tiered_transfer">Tiered transfer</option>}</select></label><label className={labelClass}>Minimum billed<input type="number" min={1} max={item.maxParticipants ?? 80} disabled={item.pricingBasis !== 'per_person'} value={item.pricingBasis === 'per_person' ? item.minimumPaidParticipants ?? '' : ''} placeholder={item.pricingBasis === 'per_person' ? 'Actual guests' : 'Not applicable'} onChange={(event) => updateItem(item.id, { minimumPaidParticipants: event.target.value ? Math.min(item.maxParticipants ?? 80, Math.max(1, Number(event.target.value) || 1)) : undefined })} className={`${fieldClass} disabled:cursor-not-allowed disabled:opacity-45`} /></label><label className={labelClass}>Max guests<input type="number" min={1} max={80} value={item.maxParticipants ?? ''} placeholder="No limit" onChange={(event) => { const maxParticipants = event.target.value ? Math.min(80, Math.max(1, Number(event.target.value) || 1)) : undefined; updateItem(item.id, { maxParticipants, ...(maxParticipants && item.minimumPaidParticipants && item.minimumPaidParticipants > maxParticipants ? { minimumPaidParticipants: maxParticipants } : {}) }); }} className={fieldClass} /></label><label className="flex min-h-11 items-center gap-2 text-sm font-bold xl:col-start-4"><input type="checkbox" checked={item.active} onChange={(event) => updateItem(item.id, { active: event.target.checked })} className="h-4 w-4 accent-[#11C7D9]" /> Active</label></div>)}</div><div className="mt-5 flex flex-wrap justify-end gap-3"><button onClick={() => void save()} disabled={!dirty || working} className="rounded-lg border border-white/18 px-5 py-3 text-sm font-bold disabled:opacity-40">Save draft</button><button onClick={() => void publish()} disabled={dirty || working} className="rounded-lg bg-[var(--brand-orange)] px-5 py-3 text-sm font-bold text-white disabled:opacity-40">Publish</button></div></div></AnimatedDisclosure>
  </section>;
};
...
const CompactStaffPanel: React.FC<StaffPanelProps> = ({ rows, setRows, run, preview, currentEmail }) => {
  ...
  return <section className="mt-7 max-w-6xl">
    <div className="flex items-start justify-between gap-4"><div><h2 className="text-2xl font-extrabold">Staff access</h2><p className="mt-2 text-sm text-[#F8F4E8]/58">Dashboard roles are stored here; Cloudflare controls who reaches the sign-in screen.</p></div><button onClick={() => setAdding((open) => !open)} aria-expanded={adding} aria-controls="new-staff" className="inline-flex min-h-10 shrink-0 items-center rounded-lg bg-[#0d3943] px-4 text-sm font-bold text-[#D9EEF1] hover:bg-[#124852]"><Plus className="mr-2 h-4 w-4" /> {adding ? 'Close form' : 'Add staff'}</button></div>
    <div className="mt-5 flex flex-col gap-2 rounded-xl bg-[#0a3039] p-4 text-sm leading-relaxed text-[#D9EEF1] sm:flex-row sm:items-center sm:justify-between"><p><strong>Two steps are required:</strong> save the person here, then add the same email to the Cloudflare Access allow policy.</p><a href="https://dash.cloudflare.com/9afab2d5eabc5a0cee88b9ecc5d2e795/one/access-controls/apps/self-hosted/c10682ba-3d82-443b-9834-e75cd2c95c15/edit?tab=basic-info" target="_blank" rel="noreferrer" className="shrink-0 font-bold text-[#BDF5FA] underline decoration-[#11C7D9]/50 underline-offset-4 hover:text-white">Open Access policy</a></div>
